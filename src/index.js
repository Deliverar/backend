...
app.put("/api/usuarios/:cn", async (req, res) => {
  ...
  ldapClient.bind(adminDN, adminPassword, (bindError) => {
    ...
    ldapClient.modify(
      `cn=${cn},ou=users,dc=deliverar,dc=com`,
      modifyChanges,
      (modifyError) => {
        if (modifyError) {
          console.error(
            "Error al modificar usuario en el servidor LDAP:",
            modifyError
          );
          res
            .status(500)
            .send("Error al modificar usuario en el servidor LDAP");
        } else {
          console.log("Usuario modificado con éxito en el servidor LDAP");
          res
            .status(200)
            .send("Usuario modificado con éxito en el servidor LDAP");
        }
        ldapClient.unbind();
      }
    );
      ...
      ldapClient.bind(usuarioDN, pass, (err) => {
        ...
        stompClient.publish({
          ...
          body: JSON.stringify({
            ...
            data: {
              //username: usuarioDN.cn,//probe con todas estas variables y no funciona : usuarioDN, usuarioDN.userName, usuarioDN.name
              email: uid, //ok
              //grupo: req.params.cn,//NO FUNCIONA
              evento: "login", //ok
            },
          }),
        });
  ...
  ldapClient.bind(adminDN, adminPassword, async (bindError) => {
    ...
    ldapClient.compare(
      ...
      (compareError, matched) => {
        if (compareError) {
          console.error(
            'Error al comparar el atributo "EmployeeNumber(OTP)":',
            compareError
          );
        ...
        if (matched) {
          ...
          ldapClient.modify(
            ...
            (modifyError) => {
              if (modifyError) {
                console.error(
                  'Error al borrar el atributo "EmployeeNumber":',
                  modifyError
                );
                res
                  .status(500)
                  .send('Error al borrar el atributo "EmployeeNumber"');
              } else {
    ...
    return new Promise((resolve, reject) => {
      ldapClient.search(
        ...
        (searchError, searchResponse) => {
          ...
          searchResponse.on("end", () => {
            console.log(
              "Búsqueda LDAP completada. Total de grupos encontrados:",
              grupos.length
            );
            ldapClient.unbind();
            resolve(grupos);
  ...
  ldapClient.bind(adminDN, adminPassword, (bindErr) => {
    ...
    ldapClient.modify(grupoDN, change, (modifyErr) => {
      ...
      if (modifyErr) {
        return res
          .status(500)
          .json({ error: "Error al agregar el usuario al grupo" });
      }

      res
        .status(200)
        .json({ message: `Usuario '${usuario}' agregado al grupo.` });
    });
  });
});
  ...
  ldapClient.bind(adminDN, adminPassword, (bindErr) => {
    ...
    ldapClient.modify(grupoDN, change, (modifyErr) => {
      ...
      if (modifyErr) {
        return res
          .status(500)
          .json({ error: "Error al eliminar el usuario del grupo" });
      }

      res
        .status(200)
        .json({ message: `Usuario '${usuario}' eliminado del grupo.` });
    });
  });
});
...
app.get("/api/grupos-ldap", (req, res) => {
  ...
  ldapClient.bind(adminDN, adminPassword, (err) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ error: "Error de conexión al servidor LDAP" });
    }
    ...
    ldapClient.search(searchBase, searchOptions, (searchErr, searchRes) => {
      if (searchErr) {
        ...
        return res
          .status(500)
          .json({ error: "Error de búsqueda en el servidor LDAP" });
      }
...
app.get("/api/miembrosGrupo", (req, res) => {
  ...
  ldapClient.bind(adminDN, adminPassword, (err) => {
    ...
    ldapClient.search(searchBase, searchOptions, (searchErr, searchRes) => {
      ...
      // Procesa los resultados de la búsqueda
      searchRes.on("searchEntry", (entry) => {
        const miembrosAttribute = entry.attributes.find(
...
app.get("/api/usuarios-de-grupo/:groupName", (req, res) => {
  ...
  ldapClient.bind(adminDN, adminPassword, (bindErr) => {
    if (bindErr) {
      ...
      return res
        .status(500)
        .json({ error: "Error de autenticación al servidor LDAP" });
    }
    ...
    ldapClient.search(searchBase, searchOptions, (searchErr, searchRes) => {
      if (searchErr) {
        console.error(
          "Error al realizar la búsqueda en el servidor LDAP:",
          searchErr
        );
        ldapClient.unbind();
        return res
      ...
      searchRes.on("end", () => {
        ...
        console.log(
          `Miembros encontrados en el grupo "${groupName}":`,
          groupMembers
        );

        // Ahora, construye un filtro de búsqueda para los miembros del grupo
        const userSearchFilter = `(|${groupMembers
          .map((userName) => `(uid=${userName})`)
          .join("")})`;
        ...
        ldapClient.search(
          searchBase,
          userSearchOptions,
          (userSearchErr, userSearchRes) => {
            if (userSearchErr) {
              console.error(
                "Error al buscar usuarios del grupo:",
                userSearchErr
              );
              ldapClient.unbind();
              return res
                .status(500)
                .json({ error: "Error al buscar usuarios del grupo" });
            }
            ...
            userSearchRes.on("end", () => {
              ...
              console.log(
                "Búsqueda en el servidor LDAP finalizada. Usuarios encontrados:",
                users.length
              );
...
app.post("/api/payment/pay", async (req, res) => {
  ...
  //Fin Evento pago de sueldo
  res.status(200).send("Pago Enviado");
});
...
async function sendEmailBrevo(otp, uid, givenName) {
  ...
  apiKey.apiKey =
    "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";
...
`;
  sendSmtpEmail.sender = {
    name: "DeliverarLDAP",
    email: "abmpersonalinternodeliverar@gmail.com",
  };
  sendSmtpEmail.to = [{ email: uid, name: "sample-name" }];
...
async function sendEmail(otp, uid, givenName) {
  ...
  console.log(
    "Correo electrónico enviado con éxito. URL de vista previa:",
          ...
          (searchError, searchResponse) => {
            ...
            searchResponse.on("end", () => {
              ...
              if (!usuarioEncontrado) {
                resolve(
                  new Error(`No se encontró un usuario con el UID: ${uid}`)
                );
              }
            });
          }
  ...
  return new Promise((resolve, reject) => {
    ...
    ldapClient.bind(adminDN, adminPassword, (bindErr) => {
      ...
      ldapClient.search(dn, searchOptions, (searchErr, res) => {
        ...
        res.on("searchEntry", (entry) => {
          console.log("PASO 1");
          const stValue = entry.attributes.find((attr) => attr.type === "st")
            .values[0];
